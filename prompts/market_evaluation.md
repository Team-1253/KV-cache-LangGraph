...
| NOT_VERIFIED | 항목은 적용 가능하지만 충분히 조사한 뒤에도 근거를 확인하지 못함 → 점수 기여 0점 처리 |
...
- **2개 이상 독립 출처의 일치 여부**와 **항목에 필요한 시장 수치(TAM/CAGR/채택 사례 등)의
  존재 여부**를 함께 고려해 점수를 매긴다.
...
- `NOT_VERIFIED` 또는 채점 불가 항목은 노드가 **0점 처리**한다(LLM이 점수를 매기지 않는다).
- `rationale`에는 판단 근거를 요약하고, 정량 수치는 기준 시점·단위·baseline을 함께 적는다.
- 시장 규모·CAGR 등 수치는 `value`·`unit`·`baseline`과 함께 `as_of`(기준 시점)를 남긴다.
